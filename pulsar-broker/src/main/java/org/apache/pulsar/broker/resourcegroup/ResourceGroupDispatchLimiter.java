...
public class ResourceGroupDispatchLimiter {
    ...
    private static final long ratePeriodNanos = TimeUnit.SECONDS.toNanos(1);
    ...
    private void update(long dispatchRateInMsgs, long dispatchRateInBytes) {
        if (dispatchRateInMsgs > 0) {
            this.dispatchRateLimiterOnMessage =
                    AsyncTokenBucket.builder().rate(dispatchRateInMsgs).ratePeriodNanos(ratePeriodNanos)
                            .build();
        } else {
        ...
        if (dispatchRateInBytes > 0) {
            this.dispatchRateLimiterOnByte =
                    AsyncTokenBucket.builder().rate(dispatchRateInBytes).ratePeriodNanos(ratePeriodNanos)
                            .build();
        } else {
